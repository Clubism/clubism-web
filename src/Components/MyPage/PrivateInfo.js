...
const PrivateInfo = ()=> {
  axios.get("http:localhost:4000/auth/test").then((res)=>{
    console.log(res.data);
  });
  var [majors, setMajors] = useState([]);
  const [Info, SetInfo] = useState({}); // Info : db에서 찾아온 user 정보
  const [Form, setForm] = useState({ // Form : 프론트 단에서 사용자에 의해 작성되는 폼
    id : "",
    password : "",
    password2 : "",
    username : "",
  ...
  useEffect(()=>{
    setForm({...Form, id : Info.id, username : Info.username, major : Info.major});
  }, [Info]);
  ...
  return (
    <PrivateInfoWrapper>
      ...
      <div style={{margin : "1.6rem 0"}}></div>
      <PrivateInfoContainer>

        <InputContainer>
          <Label for="username">이름</Label>
          <Input name="username" placeholder={Info.username} onChange={(e)=>onFormChange(e)}/>
          <Line />
        </InputContainer>
        ...
        <InputContainer>
          ...
          <Input type="password" name="password" onChange={(e)=>onFormChange(e)}/>
          <Line />
        </InputContainer>

        <InputContainer>
          <Label for="password2">비밀번호 확인</Label>
          <Input type="password" name="password2" onChange={(e)=>onFormChange(e)}/>
          <PwMsg show={Form.password === '' && Form.password2 === ''? "none" : "inline-block"}>{(Form.password === Form.password2) ? "비밀번호가 일치합니다" : "비밀번호가 일치하지 않습니다"}</PwMsg>
          <Line />
        </InputContainer>
        ...
        <InputContainer>
          <Label for="submajor">부전공</Label>
          {Info.subMajor === undefined ? <Select>
          <option value="">부전공을 선택하세요</option>
          {majors.map((m)=>{
            return (
              <option name="subMajor" value={m.label} onChange={(e)=>onFormChange(e)}>{m.name}</option>
            )
          })}
          </Select> : <Input type="text" name="major" value={Info.subMajor} disabled/>}
